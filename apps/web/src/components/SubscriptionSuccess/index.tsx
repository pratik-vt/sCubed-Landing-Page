...
import { CheckCircle2, Home, AlertCircle, Mail, Clock } from 'lucide-react';
...
export default function SubscriptionSuccess() {
  ...
  const verifySuccess = async () => {
    ...
    if (sessionId === '{CHECKOUT_SESSION_ID}') {
      setError('Payment session not properly initialized. Please contact support.');
      setVerified(false);
      setLoading(false);
      return;
    }
    ...
    if (sessionId) {
      try {
        ...
        if (!data || typeof data !== 'object') {
          setError('Invalid response from payment verification. Please contact support.');
          setVerified(false);
        } else {
        ...
        if (isApiError(err)) {
          ...
          if (err.statusCode === 404) {
            ...
            setError('Invalid or expired checkout session. Redirecting to homepage...');
            setTimeout(() => {
              router.push('/');
            }, 3000);
          } else {
  ...
  if (verified) {
    ...
    const isPaymentPending = subscriptionData.isPaidPlan && !subscriptionData.paymentVerified;

    return (
      <div className={styles.pageWrapper}>
        <div className={styles.container}>
          <div className={styles.successContainer}>
            <div className={isPaymentPending ? styles.loadingIconWrapper : styles.successIcon}>
              {isPaymentPending ? (
                <Clock size={48} />
              ) : (
            ...
            <div className={`${styles.alertContainer} ${isPaymentPending ? styles.alertWarning : styles.alertInfo}`}>
              ...
              <div>
                ...
                <ul className={styles.successList}>
                  {subscriptionData.isPaidPlan ? (
                    isPaymentPending ? (
                      <>
                        <li className={styles.successListItem}>Your payment is currently being processed by our payment provider</li>
                        <li className={styles.successListItem}>
                          You will receive a confirmation email at{' '}
                          <strong>{subscriptionData.email}</strong> once payment is confirmed
                        </li>
                        <li className={styles.successListItem}>This typically takes 1-5 minutes but may take up to 24 hours in rare cases</li>
                        <li className={styles.successListItem}>If you don't receive confirmation within 24 hours, please contact our support team</li>
                      </>
                    ) : (
                      <>
                        <li className={styles.successListItem}>Our team is setting up your account and will activate it within 1-2 business days</li>
                        <li className={styles.successListItem}>
                          You'll receive a confirmation email at{' '}
                          <strong>{subscriptionData.email}</strong> with your login credentials
                        </li>
                        <li className={styles.successListItem}>Once activated, you can log in and start using all S Cubed features immediately</li>
                        <li className={styles.successListItem}>A detailed receipt for your purchase has been sent to your email</li>
                        <li className={styles.successListItem}>If you have any questions, our support team is here to help</li>
                      </>
                    )
                  ) : (
                    <>
              ...
              <div className={styles.successDetails}>
                ...
                {subscriptionData.status && (
                  <div className={styles.successDetailsItem}>
                    ...
                    <span className={styles.value}>
                      {subscriptionData.status.replace(/_/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase())}
                    </span>
                  </div>
                )}
                {subscriptionData.paymentStatus && (
                  <div className={styles.successDetailsItem}>
                    ...
                    <span className={`${styles.value} ${isPaymentPending ? styles.paymentPending : styles.paymentCompleted}`}>
                      {subscriptionData.paymentStatus.replace(/_/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase())}
                    </span>
                  </div>
                )}
              </div>
