...
async function setupPermissions() {
  try {
    ...
    const publicRole = rolesResponse.data.roles.find(
      (role) => role.type === 'public',
    );
    ...
    try {
      ...
      console.log(
        '⚠️  Blog posts API test failed - you may need to create some content first',
      );
    }

    try {
      const eventsResponse = await axios.get(`${STRAPI_URL}/api/events`);
      console.log('✅ Events API is working!');
      console.log(`📊 Found ${eventsResponse.data.data.length} events`);
    } catch (error) {
      console.log(
        '⚠️  Events API test failed - you may need to create some events first',
      );
    }
  } catch (error) {
    ...
    if (error.response?.status === 401) {
      ...
      console.log('   - Event: find, findOne');
